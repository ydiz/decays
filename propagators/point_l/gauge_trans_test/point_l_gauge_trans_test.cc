...
int main(int argc, char **argv)
{
  ...
  string output_prefix = "/hpcgpfs01/work/lqcd/qcdqedta/ydzhao/24ID_my_props/point_l_gauge_trans_test";
  string original_prop_prefix = "/hpcgpfs01/work/lqcd/qcdqedta/ydzhao/24ID_my_props/point_l"; // propagator for Umu without gauge transformation
  ...
  SU<3>::LieRandomize(pRNG, g_test, 1.0);    
  ...
  for(int traj = traj_start; traj <= traj_end; traj += traj_sep) {
    ...
    for(Coordinate point_src: point_srcs) {
      ...
      string prop2_fname = original_prop_prefix + "/" + to_string(traj)  + "/" + coor2str(point_src.toVector());
      readScidac_prop_f2d(prop2, prop2_fname);

      LatticeColourMatrixSite g_test_x0;
      peekSite(g_test_x0, g_test, Coordinate(point_src));
