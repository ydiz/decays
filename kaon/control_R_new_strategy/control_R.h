...
std::vector<T> operator+(const std::vector<T> &lhs, const std::vector<T> &rhs) {
  std::vector<T> rst(lhs.size());
  for (int i=0; i<lhs.size(); ++i) rst[i] = lhs[i] + rhs[i];
  return rst;
}

#pragma omp declare reduction(my_vec_plus : std::vector<std::vector<Complex>> : \
                             omp_out = omp_out + omp_in) \
                    initializer(omp_priv = std::vector<std::vector<Complex>>(omp_orig.size(), std::vector<Complex>(omp_orig[0].size())))
...
vector<vector<Complex>> sum_T_R(const LatticeComplex &lat, const std::vector<int> &v) { // v is a 3-element vector, center for calculating spatial distance
  ...
  // thread_for(ss, lat.Grid()->lSites(), {// FIXME: cannot use thread for because we add to rst[t][int(R)]
  #pragma omp parallel for reduction(my_vec_plus : rst)
