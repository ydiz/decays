...
void typeIV_D1b(const std::vector<int> &u, int tK, std::vector<std::vector<Complex>> &rst_Q1_xt_vt, 
  ...
  for(int xt=0; xt<T; ++xt) {
    for(int vt=0; vt<T; ++vt) {
      rst_Q1_xt_vt[xt][vt] = trace(f_Q1_K_xt[xt] * g_vt[vt] + f_Q1_Kbar_xt[xt] * adj(g_vt[vt]))()()();  // add contribution of K0bar
      ...
      sBar_d_T2D1b_xt_vt[xt][vt] = 2. * real(sBar_d_T2D1b_xt_vt[xt][vt]);  // add contribution of K0bar
...
void typeIV_D2b(const std::vector<int> &u, int tK, std::vector<std::vector<Complex>> &rst_Q1_xt_vt, 
  ...
  for(int xt=0; xt<T; ++xt) {
    for(int vt=0; vt<T; ++vt) {
      ...
      sBar_d_T2D2b_xt_vt[xt][vt] = 2. * real(sBar_d_T2D2b_xt_vt[xt][vt]); // add contribution of K0bar
