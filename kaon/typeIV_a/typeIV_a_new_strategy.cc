...
int main(int argc, char* argv[])
{
  ...
  for(int traj = traj_start; traj <= traj_end; traj += traj_sep) {
    ...
    vector<vector<vector<vector<Complex>>>> table4d_Q1 = control_R_initialize_typeV_table4d(fdims); // table4d[tK][xt][R][vt]
    vector<vector<vector<vector<Complex>>>> table4d_Q2 = control_R_initialize_typeV_table4d(fdims);
    vector<vector<vector<vector<Complex>>>> table4d_sBar_d = control_R_initialize_typeV_table4d(fdims);
    ...
    for(const auto &v: env.xgs_l) {
      ...
      for(int tK=0; tK<T; ++tK) {
        ...
        vector<vector<Complex>> rst_Q1_xt_R, rst_Q2_xt_R, rst_sBar_d_xt_R;
        rst_Q1_xt_R = sum_T_R(rst_Q1, v);
        rst_Q2_xt_R = sum_T_R(rst_Q2, v);
        rst_sBar_d_xt_R = sum_T_R(rst_sBar_d, v);
        ...
        for(int xt=0; xt<T; ++xt) {
          ...
          for(int R=0; R<num_R; ++R) {
            ...
            table4d_sBar_d[tK][xt][R][vt] += rst_sBar_d_xt_R[xt][R] / double(pt_counts_slices[vt]);
          }
        }
      }
    ...
    std::cout << "traj [" << traj << "] table2d sBar_d T2 diagram a: " << table2d_sBar_d << std::endl;
