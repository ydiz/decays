...
// EM factor on whole lattice; if r_t < -max_uv_sep or r_t > max_uv_sep, Euv = 0
// The factor 2 / M_K^4 is included.
void EM_factor(LatticeKGG &lat, const std::vector<int> &v, double M_K, int max_uv_sep) { // max sep is maximum allowed |u0-v0|
  ...
  parallel_for(int ss=0; ss<lat.Grid()->lSites(); ss++) {
    ...
    if(gcoor[Tdir] < -max_uv_sep || gcoor[Tdir] > max_uv_sep) continue; // Non-zero only for 0 <= t <= max_uv_sep
